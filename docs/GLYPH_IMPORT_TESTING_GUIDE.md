...
The hidden panel appears under the Score Practice controls. On mobile it starts collapsed; tap `Import` to open it and `Hide` to return to the practice view. The drawer includes a live glyph preview rendered with the bundled Neanes font; tap a preview cluster to select the matching source text.
...
7. Confirm the live preview updates as you type or tap glyph buttons.
8. Insert an unknown token such as `notAGlyph`, press `Load`, and confirm diagnostics appear without replacing the active score.
...
The live preview is generated by `web/score/glyph_render.js` and covered by `web/score/tests/glyph_render.test.mjs`.
...
node --check web/score/glyph_render.js
