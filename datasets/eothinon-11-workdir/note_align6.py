...
DIGORGON = {0xf044}  # divides 3 notes (prev, carrier, next) into 1 beat, 1/3 each (chanter 2026-08-17)
...
SYNELAF = 0xf050
KENTIMATA_COMPOUNDS = {0xf0d7, 0xf06f, 0xf04f, 0xf055, 0xf059, 0xf075}  # two sung notes; 0xf077 = ONE note (petasti+oligon)
MANUAL = [(50, 54.0), (53, 58.0), (57, 61.0), (64, 65.0), (78, 76.0), (79, 77.5),
          (122, 119.0), (155, 154.0), (162, 157.0), (224, 220.97), (258, 253.0)]
...
# gorgons of the stacked yporrhoe (0xf029) have zero-width origins ~24px right
# of the glyph origin — past its x1 and at/inside the NEXT glyph — so nearest-
# interval attachment misassigns them. Chanter: 0xf029 carries the red gorgon.
...
for mi, m in enumerate(mods):
    if m['cp'] in GORGON or m['cp'] in DIGORGON or m['cp'] in DURATION_CP:
        if mi in ATTACH_OVR:
            best = ATTACH_OVR[mi]
            assert notes[best]['cp'] == 0xf029, (mi, best)
        else:
            best, bd = None, 30
...
S = len(slot_gi)
assert _digor_hits == [227], f'digorgon should fire exactly once on gi 227: {_digor_hits}'
