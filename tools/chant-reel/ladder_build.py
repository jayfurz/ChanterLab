...
for fi in range(n_frames):
    ...
    flat = 0
    if attr == 'kezo':
        # first-crossing rule: what happens first decides — descent to the Di
        # thesis => Zo-flat; rise back above Zo => still arsis => raised Ke
        name = 'Ζω'; flat = 1          # default: completing descent
        for k in range(i0, min(i0+150, len(mor))):
            v = mor[k]
            if np.isnan(v): continue
            v -= root_offset(k/100.0)
            if v <= 47: break                      # thesis descent confirmed
            if v >= 62: name, flat = 'Κε↑', 0; break   # rose back: arsis
