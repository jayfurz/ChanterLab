...
parallagi onset across the path. Two controls are scored beside it:
  ...
  dtw       pitch-contour DTW, onsets carried across the path

Scored by onset_eval.py against the chanter's pins on the melos. A model is
worth building only if the gap between dtw and the 150 ms gate is real.
...
def contour(path):
    """Cents relative to the piece's own median F0; unvoiced frames held."""
    f0, ok = f0_track(audio(path))
    f = f0.copy()
    if ok.sum() < 10:
        return np.zeros(len(f0)), ok
    med = np.median(f0[ok])
    c = 1200.0 * np.log2(np.maximum(f, 1.0) / med)
    c[~ok] = np.nan
    # hold the last voiced value through unvoiced frames
    last = 0.0
...
def dtw_path(a, b, band=None):
    """Plain DTW on 1-D sequences with a Sakoe-Chiba band; returns the path."""
    n, m = len(a), len(b)
...
def main():
    ...
    cp, _ = contour(os.path.join(a.par, 'audio.wav'))
    cm, _ = contour(os.path.join(a.mel, 'audio.wav'))
    fp, fm = int(sp * SR / HOP), int(sm * SR / HOP)
    cp, cm = cp[fp:], cm[fm:]
    dp, dm = len(cp) * HOP / SR, len(cm) * HOP / SR
    print('parallagi %.1fs from %.1fs, melos %.1fs from %.1fs, ratio %.3f, %d parallagi onsets'
          % (dp, sp, dm, sm, dm / dp, len(po)))
    ...
    for nm, pred in (('stretch', stretch), ('dtw', dtw)):
        f = '%s_%s.json' % (out, nm)
